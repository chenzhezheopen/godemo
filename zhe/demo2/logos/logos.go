...
func main() {
	...
	//Flags返回Logger的输出选项
	fmt.Println("Logger的输出选项", loger.Flags())

	//SetFlags设置输出选项
	loger.SetFlags(log.Ldate | log.Ltime | log.Lshortfile)

	//返回输出前缀
	fmt.Println("返回输出前缀", loger.Prefix())
	...
	//获取输出项
	fmt.Println("获取输出项", log.Flags())
	//获取前缀
	fmt.Printf("获取前缀", log.Prefix())
	//输出内容
	log.Output(2, "输出内容")
	//格式化输出
	...
	logFil, arr := os.Open("20190308.txt")
	if arr != nil {
		fmt.Println("读取日志文件失败")
