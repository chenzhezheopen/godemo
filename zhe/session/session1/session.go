...
//StartSession 在开始页面登陆页面，开始Session
func (mgr *SessionMgr) StartSession(w http.ResponseWriter, r *http.Request) string {
	...
	fmt.Println(mgr.mSessions)
	//让浏览器cookie设置过期时间
	cookie := http.Cookie{Name: "666", Value: newSessionID, Path: "/", HttpOnly: true, MaxAge: int(mgr.mMaxLifeTime)}
...
func (mgr *SessionMgr) CheckCookieValid(w http.ResponseWriter, r *http.Request) string {
	...
	var cookie, err = r.Cookie("2222")
	Logger.Println("cookie is :", cookie, "err is :", err)
	if cookie == nil || err != nil {
		Logger.Println("CheckCookieValid err 01")
		return ""
	}
	mgr.mLock.Lock()
	defer mgr.mLock.Unlock()
	...
	fmt.Println("tuichu")
	Logger.Println("CheckCookieValid err 02")
	return ""
}
